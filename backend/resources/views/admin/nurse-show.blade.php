...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    <div class="lg:col-span-2 bg-white rounded-lg shadow p-6">
        <div class="flex items-center justify-between mb-4">
            ...
            <a href="{{ route('admin.nurses.edit', $nurse) }}"
        ...
        <dl class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
            ...
            <div class="md:col-span-2">
                ...
                <dd class="font-semibold">
                    {{ is_array($profile?->competence_areas) && count($profile->competence_areas) ? implode(', ',
                    $profile->competence_areas) : 'N/A' }}
                </dd>
            </div>
            <div class="md:col-span-2">
    ...
    <div class="space-y-6">
        <div class="bg-white rounded-lg shadow p-6">
            ...
            <div class="space-y-3">
                <form method="POST" action="{{ route('admin.nurses.verify', $nurse) }}">
                    @csrf
                    <button class="w-full bg-green-600 text-white py-2 rounded hover:bg-green-700" type="submit">Verify
                        Nurse</button>
                </form>
                ...
                <form method="POST" action="{{ route('admin.nurses.reject', $nurse) }}">
                    ...
                    <button class="w-full bg-orange-600 text-white py-2 rounded hover:bg-orange-700"
                        type="submit">Decline Application</button>
                </form>
                @endif
                ...
                <form method="POST" action="{{ route('admin.nurses.activate', $nurse) }}">
                    ...
                    <button class="w-full bg-emerald-600 text-white py-2 rounded hover:bg-emerald-700"
                        type="submit">Activate Nurse</button>
                </form>
                ...
                <form method="POST" action="{{ route('admin.nurses.delete', $nurse) }}"
                    ...
                    @csrf
                    @method('DELETE')
                    <button class="w-full bg-rose-600 text-white py-2 rounded hover:bg-rose-700" type="submit">Delete
                        Nurse</button>
                </form>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow p-6">
            ...
            <form method="POST" action="{{ route('admin.nurses.suspend', $nurse) }}" class="space-y-3">
                ...
                <div>
                    ...
                    <textarea name="reason" class="w-full border rounded px-3 py-2" rows="2"
                ...
                <button class="w-full bg-yellow-600 text-white py-2 rounded hover:bg-yellow-700" type="submit">Suspend
                    Nurse</button>
            </form>
        </div>
        ...
        <div class="bg-white rounded-lg shadow p-6">
            ...
            <form method="POST" action="{{ route('admin.nurses.ban', $nurse) }}" class="space-y-3">
                ...
                <button class="w-full bg-red-600 text-white py-2 rounded hover:bg-red-700" type="submit">Ban
                    Nurse</button>
            </form>
        </div>
    </div>
