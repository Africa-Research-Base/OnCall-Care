...
export default function ActiveJobScreen({ navigation, route }) {
  ...
  useEffect(() => {
    const interval = setInterval(async () => {
      try {
        ...
        const response = await fetch(`${API_URL}/request/${patient.id}`, {
  ...
  }, [hasCancellationAlerted, navigation, patient.id]);
  ...
  const handleChat = async () => {
    ...
    await Linking.openURL(`sms:${patient.phone}`);
  };

  const handleDirections = async () => {
    const destination = `${patientLoc.latitude},${patientLoc.longitude}`;
