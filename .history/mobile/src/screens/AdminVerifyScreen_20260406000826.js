...
export default function AdminVerifyScreen() {
  ...
  const fetchNurses = useCallback(async () => {
    ...
    setNurses(Array.isArray(nursesData?.data) ? nursesData.data.map(toUserRow) : []);
  }, []);
  ...
  const openDetails = async (nurse) => {
    try {
      setSaving(true);
      const headers = await getAuthHeaders();
      const response = await fetch(`${API_URL}/admin/nurses/${nurse.id}`, { headers });
      const data = await response.json();

      if (!response.ok) {
        Alert.alert("Load Failed", data?.message || "Could not load nurse details.");
        return;
      }
  ...
  const saveNurseEdits = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/admin/nurses/${selectedNurse.id}`, {
        method: "PUT",
        headers,
      ...
      const data = await response.json();
      if (!response.ok) {
        Alert.alert("Update Failed", data?.message || "Could not update nurse.");
        return;
      }
  ...
  const runNurseAction = async (path, body = null) => {
    ...
    try {
      setSaving(true);
      const headers = await getAuthHeaders(true);
      const response = await fetch(`${API_URL}/admin/nurses/${selectedNurse.id}/${path}`, {
        method: "POST",
        headers,
  ...
  const deleteNurse = async () => {
    ...
    try {
      ...
      const data = await response.json();
      if (!response.ok) {
        Alert.alert("Delete Failed", data?.message || "Could not delete nurse.");
        return;
      }
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.tabRow}>
        ...
        <TouchableOpacity
          style={[styles.tabButton, activeTab === "nurses" && styles.tabButtonActive]}
          onPress={() => setActiveTab("nurses")}
        >
          <Text style={[styles.tabText, activeTab === "nurses" && styles.tabTextActive]}>
            Nurses
          </Text>
        </TouchableOpacity>
      </View>
      ...
      {activeTab === "applications" ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={<Text style={styles.empty}>No nurses found.</Text>}
          renderItem={({ item }) => (
            <View style={styles.card}>
              <View style={styles.info}>
                <Text style={styles.name}>{item?.name || "Unknown Nurse"}</Text>
                <Text style={styles.meta}>{item?.email}</Text>
                <Text style={styles.meta}>Status: {item?.nurse_profile?.account_status || "active"}</Text>
                <Text style={styles.meta}>Verified: {item?.nurse_profile?.is_verified ? "Yes" : "No"}</Text>
              </View>
          ...
          <View style={styles.modalContent}>
            ...
            <ScrollView>
              ...
              <View style={styles.modalActionsGrid}>
                <TouchableOpacity style={[styles.actionBtn, styles.approveBtn]} onPress={saveNurseEdits}>
                  <Text style={styles.actionText}>Save</Text>
                </TouchableOpacity>
                <TouchableOpacity style={[styles.actionBtn, styles.verifyBtn]} onPress={() => runNurseAction("verify")}>
                ...
                <TouchableOpacity
                  ...
                  onPress={() => runNurseAction("ban", { reason: actionReason || undefined })}
                >
                  <Text style={styles.actionText}>Ban</Text>
                </TouchableOpacity>
                <TouchableOpacity style={[styles.actionBtn, styles.activateBtn]} onPress={() => runNurseAction("activate")}>
                  <Text style={styles.actionText}>Activate</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  style={[styles.actionBtn, styles.rejectBtn]}
                  onPress={() =>
                    Alert.alert("Delete Nurse", "This will permanently delete the nurse account.", [
                      { text: "Cancel", style: "cancel" },
                      { text: "Delete", style: "destructive", onPress: deleteNurse },
                    ])
                  }
                >
