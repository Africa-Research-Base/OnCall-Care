    ...
    public function show(Request $request, $id)
    {
        ...
        if ($medicalRequest->nurse_id) {
            ...
            if ($nurseUser) {
                ...
                if ($nurseUser->lat && $nurseUser->lng && $medicalRequest->pickup_lat && $medicalRequest->pickup_lng) {
                    $earthRadiusKm = 6371;
                    $latFrom = deg2rad((float) $nurseUser->lat);
                    $lonFrom = deg2rad((float) $nurseUser->lng);
                    $latTo = deg2rad((float) $medicalRequest->pickup_lat);
                    $lonTo = deg2rad((float) $medicalRequest->pickup_lng);
