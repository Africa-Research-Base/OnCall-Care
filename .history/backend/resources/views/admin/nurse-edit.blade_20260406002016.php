        ...
        <form method="POST" action="{{ route('admin.nurses.update', $nurse) }}" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input name="email" type="email" value="{{ old('email', $nurse->email) }}" class="w-full border rounded px-3 py-2" />
                </div>
                <div>
                ...
                <div>
                    ...
                    <input name="license_number" value="{{ old('license_number', $nurse->nurseProfile?->license_number) }}" class="w-full border rounded px-3 py-2" />
                </div>
                <div>
                    <label class="text-sm text-gray-600">License Photo URL</label>
                    <input name="license_photo_url" value="{{ old('license_photo_url', $nurse->nurseProfile?->license_photo_url) }}" class="w-full border rounded px-3 py-2" />
                </div>
                <div>
                    <label class="text-sm text-gray-600">Experience Years</label>
                    <input type="number" min="0" max="60" name="experience_years" value="{{ old('experience_years', $nurse->nurseProfile?->experience_years) }}" class="w-full border rounded px-3 py-2" />
                </div>
                <div>
                    <label class="text-sm text-gray-600">Verification Level</label>
                    <input type="number" min="1" max="3" name="verification_level" value="{{ old('verification_level', $nurse->nurseProfile?->verification_level ?? 1) }}" class="w-full border rounded px-3 py-2" />
                </div>
                <div class="md:col-span-2">
                    <label class="text-sm text-gray-600">Competence Areas (comma separated)</label>
                    <input name="competence_areas" value="{{ old('competence_areas', is_array($nurse->nurseProfile?->competence_areas) ? implode(', ', $nurse->nurseProfile->competence_areas) : '') }}" class="w-full border rounded px-3 py-2" />
                </div>
                <div>
            ...
            <div class="flex items-center justify-end gap-3">
                <a href="{{ route('admin.nurses.show', $nurse) }}" class="px-4 py-2 rounded border border-gray-300 text-gray-700 hover:bg-gray-50">Cancel</a>
