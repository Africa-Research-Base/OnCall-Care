        ...
        <table class="w-full text-left border-collapse">
            ...
            <tbody class="text-sm text-gray-700">
                ...
                <tr class="border-b hover:bg-gray-50 transition">
                    ...
                    <td class="p-4 font-mono text-xs bg-gray-50 rounded inline-block">{{ $profile?->license_number ?? 'N/A' }}</td>
                    <td class="p-4">
                    ...
                    <td class="p-4">
                        ...
                        @if($profile?->is_verified)
                            <span class="ml-1 px-2 py-1 rounded-full text-xs font-bold bg-blue-100 text-blue-700">Verified</span>
                        @endif
                    </td>
                    <td class="p-4">
                        <div class="flex items-center gap-3">
                            <a href="{{ route('admin.nurses.show', $nurse) }}" class="text-slate-600 hover:text-slate-800" title="View"><i class="fas fa-eye"></i></a>
                            <a href="{{ route('admin.nurses.edit', $nurse) }}" class="text-blue-500 hover:text-blue-700" title="Edit"><i class="fas fa-edit"></i></a>

                            @if(!($profile?->is_verified))
                                <form method="POST" action="{{ route('admin.nurses.verify', $nurse) }}">
                                    @csrf
                                    <button class="text-green-500 hover:text-green-700" title="Verify" type="submit"><i class="fas fa-check-circle"></i></button>
                                </form>

                                <form method="POST" action="{{ route('admin.nurses.reject', $nurse) }}">
                                    @csrf
                                    <button class="text-orange-500 hover:text-orange-700" title="Decline" type="submit"><i class="fas fa-times-circle"></i></button>
                                </form>
                            @endif
                            ...
                            @if($accountStatus !== 'active')
                                <form method="POST" action="{{ route('admin.nurses.activate', $nurse) }}">
                                    @csrf
                                    <button class="text-emerald-500 hover:text-emerald-700" title="Activate" type="submit"><i class="fas fa-user-check"></i></button>
                                </form>
                            @endif
                            ...
                            @if($accountStatus !== 'banned')
                                <form method="POST" action="{{ route('admin.nurses.ban', $nurse) }}">
                                    ...
                                    <button class="text-red-500 hover:text-red-700" title="Ban" type="submit"><i class="fas fa-ban"></i></button>
                                </form>
                            @endif
                            ...
                            <form method="POST" action="{{ route('admin.nurses.delete', $nurse) }}" onsubmit="return confirm('Delete this nurse account permanently?')">
                                @csrf
                                @method('DELETE')
                                <button class="text-rose-600 hover:text-rose-800" title="Delete" type="submit"><i class="fas fa-trash"></i></button>
                            </form>
                        </div>
