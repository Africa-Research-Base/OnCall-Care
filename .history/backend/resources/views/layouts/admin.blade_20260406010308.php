        ...
        <div class="w-64 bg-slate-800 text-white flex flex-col">
            ...
            <nav class="flex-1 p-4 space-y-2">
                <a href="{{ route('admin.dashboard') }}"
                    class="block p-3 rounded hover:bg-slate-700 {{ request()->routeIs('admin.dashboard') ? 'bg-slate-700 text-red-400' : '' }}">
                    <i class="fas fa-chart-line w-6 text-center"></i> Dashboard
                </a>
                <a href="{{ route('admin.activities') }}"
                    class="block p-3 rounded hover:bg-slate-700 {{ request()->routeIs('admin.activities') ? 'bg-slate-700 text-red-400' : '' }}">
                    <i class="fas fa-list-alt w-6 text-center"></i> Activities
                </a>
                <a href="{{ route('admin.requests.cancelled') }}"
                    class="block p-3 rounded hover:bg-slate-700 {{ request()->routeIs('admin.requests.cancelled') ? 'bg-slate-700 text-red-400' : '' }}">
                    <i class="fas fa-ban w-6 text-center"></i> Cancelled Requests
                </a>
                <a href="{{ route('admin.nurses') }}"
                    class="block p-3 rounded hover:bg-slate-700 {{ request()->routeIs('admin.nurses') ? 'bg-slate-700 text-red-400' : '' }}">
